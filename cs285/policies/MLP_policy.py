...
class MLPPolicy(BasePolicy):
    ...
    def get_action(self, obs):
        ...
        # DONE:
        # T return the action that the policy prescribes
        # H1: you will need to call self.sess.run
        # H2: the tensor we're interested in evaluating is self.sample_ac
        # H3: in order to run self.sample_ac, it will need observation fed into the feed_dict
...
class MLPPolicyPG(MLPPolicy):
    ...
    def define_train_op(self):
        ...
        # DONE
        # T: define the loss that should be optimized when training a policy with policy gradient
        ...
        self.loss = tf.reduce_sum((-1)*self.logprob_n*self.adv_n)

        # DONE T: define what exactly the optimizer should minimize when updating the policy
        self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(self.loss)

        if self.nn_baseline:
            # DONE
            # T: define the loss that should be optimized for training the baseline
            ...
            # DONE T: define what exactly the optimizer should minimize when updating the baseline
    ...
    def run_baseline_prediction(self, obs):
        # DONE:
        # T: query the neural net that's our 'baseline' function, as defined by an mlp above
    ...
    def update(self, observations, acs_na, adv_n=None, acs_labels_na=None, qvals=None):
        ...
        if self.nn_baseline:
            ...
            # DONE:
            # T: update the nn baseline with the targets_n
